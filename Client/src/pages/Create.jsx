...
function Create() {
    ...
    const generateRandomTraits = () => {
      ...
      // Ensure each trait gets at least a small value to prevent 0s
      let minTraitValue = 5;  // Minimum value per trait
      let totalMin = minTraitValue * keys.length; // Total reserved minimum
      remaining -= totalMin; 
  
      for (let i = 0; i < keys.length - 1; i++) {
          let maxAlloc = remaining / (keys.length - i);
          let value = Math.floor(Math.random() * maxAlloc) + minTraitValue; // Ensure minimum
          traits[keys[i]] = value;
          remaining -= value;
      }
  
      // Assign remaining points to the last trait
    ...
    const handlePrevStep = () => {
      if (stepCreate > 1) {
          setFade(false);
          setTimeout(() => {
              setStepCreate(prev => Math.max(1, prev - 1)); // Ensures it never drops below 1
              setFade(true);
          }, 400);
      }
  };
    ...
    return (
      ...
      />
            ...
            <div className='create-container-creta-section-detail'>
                ...
                <header>
                      <button onClick={handlePrevStep} disabled={stepCreate === 1}   ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M11.8284 12.0005L14.6569 14.8289L13.2426 16.2431L9 12.0005L13.2426 7.75781L14.6569 9.17203L11.8284 12.0005Z"></path></svg></button>
                    
                      <div className='create-progress-bar'>
                            <div className='progress' style={{ width: `${(stepCreate / totalSteps) * 100}%` }}></div>
                        </div>

                      <button onClick={handleNextStep} disabled={stepCreate === totalSteps}  ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M12.1717 12.0005L9.34326 9.17203L10.7575 7.75781L15.0001 12.0005L10.7575 16.2431L9.34326 14.8289L12.1717 12.0005Z"></path></svg></button>
               </header>
...
{stepCreate === 3 && (
     ...
     <div class="create-box-chdw3e4f">
    <AiPersona aigender={selectedGender}
     selectedAge={selectedAge}
     setSelectedAge={setSelectedAge}
                  ...
                  {selectedOption && (
                    <div className="create-box-display-overlayy">
                      {selectedOption === "existing" ? (
                        <AiExixtingCh aigender={selectedGender} onClose={() => setSelectedOption(null)} onSelect={handleImageSelect}  />

                      ) : (
                        <AiSelectCh aigender={selectedGender} onClose={() => setSelectedOption(null)}   onSelect={handleImageSelect}/>
                      )}
                    </div>
                  )}
                  
                  {selectedImage && (
        <img className='ai-charecter09-topi-image' src={selectedImage} alt="Selected" />
      )}
